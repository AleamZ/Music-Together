...
- **The RPCs:** the session-less `upsert_video_lyrics(text,…)` and `update_video_lyric_offset(text,integer,text)` are dropped. The new ones take `(p_room_id, p_session_token, …)` and accept only the room's **DJ** — the same check as play / pause / seek / skip — for an 11-character YouTube id that is the room's current or queued song, within size caps (lyrics ≤ 20 000 characters each, names ≤ 200, timing source `auto` / `custom`, offset within ±10 minutes). "Updated by" is the account's username.
