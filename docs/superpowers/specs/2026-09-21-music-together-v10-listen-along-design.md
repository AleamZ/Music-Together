...
- **Audio gate:** `unlocked` starts `false`. While `false` the engine never calls `yt.play()`. `unlock()` sets it `true` and, if `room.is_playing && current`, calls `yt.seekTo(targetSeconds(room)); yt.play()` synchronously in the same call so the gesture is honored. `unlocked` is React state only (a page reload needs a new gesture; that is how browsers work). It also flips to `true` on mount when `navigator.userActivation?.hasBeenActive` is already true — the document already had a gesture (e.g. the lobby click that entered the room), so most members never see the button.
