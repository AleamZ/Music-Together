...
| R18 | In `buy_farm_item`, a tool with a quantity other than 1 is a plain `invalid quantity`, not `bad_qty`. | The cached v15.1 shop parses `tool` rows as seeds with a stepper, so it can send 2. |
...
- **Deploy order (R28).** `0016` goes live first and the v15.2 client right after, ideally at a quiet hour.
  ...
  - A v15.2 client against a database without `0016` treats a missing `upland_crops` (PGRST205/42P01) as an empty catalog, and shows its new RPCs' PGRST202 as `NOT_OPEN_152` (§13.6).
